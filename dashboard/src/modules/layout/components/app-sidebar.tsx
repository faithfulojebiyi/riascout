...
export const AppSidebar = ({ workspaceName }: { workspaceName: string }) => {
  ...
  return (
    <Sidebar>
      <SidebarHeader>
        <div className={css({ fontSize: 'sm', fontWeight: '600', px: '2', py: '1' })}>
          {workspaceName}
        </div>
      </SidebarHeader>
              ...
              {isPending ? (
                ...
                data?.entities.map((entity) => (
                  <SidebarMenuItem key={entity.id}>
                    <SidebarMenuButton asChild isActive={pathname === `/${entity.slug}`}>
                      <Link to="/$entitySlug" params={{ entitySlug: entity.slug }}>
                        {entity.name}
                      </Link>
                    </SidebarMenuButton>
                    {entity.recordCount > 0 ? (
                      <SidebarMenuBadge>{entity.recordCount.toLocaleString()}</SidebarMenuBadge>
                    ) : null}
                  </SidebarMenuItem>
                ))
              )}
      ...
      <SidebarFooter>
        <div className={css({ color: 'text.muted', fontSize: 'xs', px: '2', py: '1' })}>
          RIAScout
        </div>
      </SidebarFooter>
    </Sidebar>
