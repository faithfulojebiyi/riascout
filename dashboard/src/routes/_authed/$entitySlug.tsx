...
import { entitiesControllerGetEntities, entitiesControllerGetEntityRecords } from '../../api/generated/entities/entities';
...
export const Route = createFileRoute('/_authed/$entitySlug')({
  loader: async ({ params }) => {
    ...
    const seed = await entitiesControllerGetEntityRecords({ entityId: entity.id, limit: 1 });
...
function EntityPage() {
  ...
  return (
    <div className={css({ display: 'flex', flexDirection: 'column', h: '100dvh' })}>
      <header
        className={css({
          alignItems: 'baseline',
      ...
      >
        <h1 className={css({ fontSize: 'lg', fontWeight: '600' })}>{entity.name}</h1>
      ...
      <div className={css({ flex: '1', minH: '0' })}>
        {view ? <EntityGrid entityId={entity.id} view={view} /> : <p>No view configured</p>}
      </div>
    </div>
  );
}
